...
#if 1
static bool mcp23x17_volatile_register(struct device *dev, unsigned int reg)
{
    unsigned int _reg = (reg >> 1);
    switch (_reg) {
    case MCP_IODIR:
    case MCP_INTF:
    case MCP_INTCAP:
...
const struct regmap_config mcp23x17_regmap = {
    ...
    #if 1
    .volatile_reg = mcp23x17_volatile_register,
    #endif
};
    ...
    unsigned int addr, unsigned int type, unsigned int base)
{
    ...
    if (ret < 0)
        return dev_err_probe(dev, ret, "can't identify chip %d\n", addr);
    pr_err("%02X", ~dir);
    //mcp_set_mask(mcp, MCP_IODIR, ~dir, false);
#endif
...
{
    devm_free_irq(dev, irq, mcp);
}
EXPORT_SYMBOL_GPL(mcp23s08_free_irq);
#endif
