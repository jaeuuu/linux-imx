...
#if 0
static const struct reg_default lp8864_defaults[] = {
    {.reg = LP8864_BRT_CONTROL, .def = 0xffff},
};

static bool lp8864_volatile_register(struct device *dev, unsigned int reg)
{
    switch (reg) {
    case LP8864_USER_CONFIG1:
    case LP8864_SUPPLY_STATUS:
    case LP8864_BOOST_STATUS:
    case LP8864_LED_STATUS:
        return true;
    default:
        return false;
    }
...
static const struct regmap_config lp8864_regmap_config = {
    ...
    #if 0
    .reg_defaults = lp8864_defaults,
    .num_reg_defaults = ARRAY_SIZE(lp8864_defaults),
    .cache_type = REGCACHE_FLAT,
    .volatile_reg = lp8864_volatile_register,
    //.reg_stride = 1,
    .max_register = LP8864_LED_STATUS,
    #endif
};
...
static int lp8864_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    int ret;
    ...
    ret = regmap_update_bits(led->regmap, LP8864_BRT_CONTROL, GENMASK(15, 0), 0xffff);
    if (ret) {
        dev_err(&led->client->dev, "Failed to set brightness control value\n");
        return ret;
    }
